...
- [ ] **[M]** `D3dxSkinManager.Client/src/shared/components/compact/CompactSwitch.css:68-71` — Unchecked hover color is hardcoded dark-only and breaks in light theme _(DEFERRED — needs a screenshot pass: the correct light-theme unchecked-hover overlay can't be picked blind; mirror the theme-split of the checked-hover rules and verify the switch hover in both themes.)_
...
- [ ] **[M]** `D3dxSkinManager/Modules/Fluent/Services/MigrationRunner.cs:93-137` — MigrateToLatestAsync and MigrateToVersionAsync are deceptively declared async but block the calling thread synchronously _(DEFERRED — sync-by-DESIGN: FluentMigrator's MigrateUp has no async API and migrations MUST run on the calling thread for SQLite transaction thread-affinity + single-connection (DATABASE_MIGRATION_ARCHITECTURE.md). The Task return only lets them compose in the async startup pipeline; wrapping in Task.Run to "make them async" would BREAK that affinity. Documented in-code so it isn't re-flagged.)_
- [ ] **[M]** `D3dxSkinManager/Modules/Context/Services/ProfilePathService.cs:119-123` — Blocking async via Task.Run + GetAwaiter().GetResult() inside a synchronous cache factory can deadlock _(DEFERRED — no deadlock: `Task.Run(...).GetAwaiter().GetResult()` runs the async work on a pool thread WITHOUT the caller's SynchronizationContext, so no continuation is posted back to the (blocked) caller — that is the standard SAFE sync-over-async shape, not the deadlocking `foo().Result` one. It blocks the caller once per cache-miss to load config for a SYNCHRONOUS property (`WorkDirectory`); making that property async would ripple to many sync call-sites. Revisit only if WorkDirectory is made async wholesale.)_
- [ ] **[M]** `D3dxSkinManager/Modules/Remote/Services/BaiduShareResolver.cs:239-253` — FindBestFile skips directories entirely, so a share whose root is a wrapper folder always fails _(DEFERRED — needs a NEW Baidu subfolder-listing call (`share/list` with `dir=` instead of `root=1`) that no code path exercises today; the remote MEGA/pan download+import path is live-validation-gated + untested in-app (remote-library.md, TASKS.md verification section), so adding an unverifiable request shape risks silent breakage — same reasoning that parked the RemoteImportService dedup in `risky-change-tests-first.md`. Do WITH the user during a live pan pass; recurse like Quark's `FindBestFileAsync` (ListDir + depth guard).)_
...
- [ ] **[M]** `D3dxSkinManager/Modules/Migration/Steps/MigrationStep5MigrateModArchives.cs:164-166` — Raw Directory.CreateDirectory on a mod archive parent path — bypasses IFileOperationPlanner _(DEFERRED — accepted migration pattern: it ensures the Mods ROOT dir exists during a ONE-SHOT migration (no concurrent live mod ops), and the archive copy on the very next line also goes direct via `_fileService`. `filesystem-operation-serialization.md` accepts migration raw-FS as LOW; the planner is for LIVE concurrency. Routing one CreateDirectory through it buys nothing + is inconsistent with the sibling copy.)_
...
- [ ] **[L]** `D3dxSkinManager/Modules/Plugin/Services/PluginInstallService.cs:218-222` — Fresh install calls LoadPluginsAsync() re-scanning all plugins, not just the new pack _(DEFERRED — LoadPluginsAsync is IDEMPOTENT (PluginRegistry.RegisterPlugin skips already-registered ids), so the re-scan is redundant-but-correct; a targeted load-one-pack method is NEW loader surface for a rare user action — poor cost/benefit. Revisit only if plugin install becomes hot.)_
