...
public class ImageService : IImageService
{
    ...
    public async Task<string?> GetThumbnailPathAsync(string sha)
    {
        ...
        foreach (var ext in _supportedExtensions)
        {
            ...
            if (File.Exists(cachedPath))
            {
                ...
                return await Task.FromResult(relativePath ?? cachedPath).ConfigureAwait(false);
            }
        }
        ...
        foreach (var fileName in previewFiles)
        {
            ...
            if (File.Exists(sourcePath))
            {
                ...
                try
                {
                    ...
                    await ResizeImageAsync(sourcePath, targetPath, ThumbnailWidth, ThumbnailHeight).ConfigureAwait(false);
    ...
    public async Task<string?> GetThumbnailAsDataUriAsync(string sha)
    {
        ...
        foreach (var ext in _supportedExtensions)
        {
            ...
            if (File.Exists(cachedPath))
            {
                return await GetImageAsDataUriAsync(cachedPath).ConfigureAwait(false);
            }
        }
    ...
    public async Task<List<string>> GetPreviewsAsDataUriAsync(string sha)
    {
        ...
        var previewPaths = await GetPreviewPathsAsync(sha).ConfigureAwait(false);

        foreach (var previewPath in previewPaths)
        {
            var dataUri = await GetImageAsDataUriAsync(previewPath).ConfigureAwait(false);
            if (dataUri != null)
            {
