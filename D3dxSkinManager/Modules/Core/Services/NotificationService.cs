...
public interface INotificationService
{
    ...
    /// </summary>
    event EventHandler<Notification>? OperationNotificationReceived;
}
...
public class NotificationService : INotificationService
{
    ...
    public event EventHandler<Notification>? OperationNotificationReceived;

    public NotificationService(ILogHelper logger)
    {
    ...
    public IProgressReporter CreateOperation(string operationName, object? metadata = null)
    {
        ...
        EmitNotification(NotificationType.OperationStarted, operation);
    ...
    public Task CancelOperationAsync(string operationId)
    {
        if (_activeOperations.TryGetValue(operationId, out var operation))
        {
            ...
            EmitNotification(NotificationType.OperationCancelled, operation);
    ...
    internal Task UpdateProgressAsync(string operationId, int percentComplete, string? currentStep)
    {
        if (_activeOperations.TryGetValue(operationId, out var operation))
        {
            ...
            EmitNotification(NotificationType.ProgressUpdate, operation);
        }
    ...
    internal Task CompleteOperationAsync(string operationId)
    {
        if (_activeOperations.TryGetValue(operationId, out var operation))
        {
            ...
            EmitNotification(NotificationType.OperationCompleted, operation);
    ...
    internal Task FailOperationAsync(string operationId, string errorMessage)
    {
        if (_activeOperations.TryGetValue(operationId, out var operation))
        {
            ...
            EmitNotification(NotificationType.OperationFailed, operation);
    ...
    /// </summary>
    private void EmitNotification(NotificationType type, OperationProgress operation)
    {
    ...
    private class OperationProgressReporter : IProgressReporter
    {
        ...
        public OperationProgressReporter(string operationId, NotificationService service)
        {
