...
| `Plugin` | PluginLoader/PluginRegistry/PluginContext; PluginManagementService (enable/disable + list — the facade delegates, never carries plugin-init logic); packs load from `{profile}/plugins/`; catalog pulled live from the SEPARATE plugin repo (`PluginInstallService`, no hard-coded list); Core+Plugin.Sdk = vendored contracts (`plugin-system.md`) |
