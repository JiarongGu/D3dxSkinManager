...
- **`compact/` (L1 atoms):** CompactButton, CompactInput, CompactSelect, CompactSwitch, CompactText, CompactCard, CompactAlert, CompactDivider, CompactSpace, CompactSection, **CompactField** (labeled form row), **CompactIconButton** (toned inline icon action), CompactUpload, CompactThumbnailUpload
- **`common/` (L1/L2):** SlideInScreen, DataTable, CloseButton, CountBadge, StatusIcon, HealthStatusIcon, **StatusTag** (semantic status pills), TooltipSystem (AnnotationProvider/AnnotatedTooltip), **KeyCaptureInput** (3DMigoto hotkey chord capture)
