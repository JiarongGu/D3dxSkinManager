#!/usr/bin/env node
// test-update-apply.mjs — end-to-end test of the C++ launcher's APPLY phase, for the lib/ topology.
//
...
//      lib/D3dxSkinManager.App.exe). Staged payload updates the runtime + a data file, adds one, drops
...
//      the launcher; runtime moved to lib/D3dxSkinManager.App.exe). Running the OLD launcher must: copy
//      the new launcher over the old runtime's D3dxSkinManager.exe, add lib/, AND — the crux — NOT delete
...
total += scenario('new self-update', 'D3dxSkinManager.exe',
  (install) => {
    // Installed (v2.4): runtime in lib\ (A), data/en.json (X), data/old.json (removed later).
    write(install, 'lib/D3dxSkinManager.App.exe', 'APP-A');
    write(install, 'data/en.json', 'EN-X');
    write(install, 'data/old.json', 'OLD');
    write(install, 'manifest.json', manifest('2.4', [
      ['lib/D3dxSkinManager.App.exe', 'APP-A'],
      ['data/en.json', 'EN-X'],
      ['data/old.json', 'OLD'],
    ]));
    // Staged (v2.5): runtime A2, en.json X2, new.json added, + a newer launcher (must be excluded).
    write(install, '.update/staged/lib/D3dxSkinManager.App.exe', 'APP-A2');
    ...
    write(install, '.update/staged/manifest.json', manifest('2.5', [
      ['lib/D3dxSkinManager.App.exe', 'APP-A2'],
      ['data/en.json', 'EN-X2'],
      ['data/new.json', 'NEW'],
    ]));
  ...
  (read, exists) => [
    ['runtime updated (lib/App.exe = A2)', exists('lib/D3dxSkinManager.App.exe') && read('lib/D3dxSkinManager.App.exe') === 'APP-A2'],
...
total += scenario('migration old-to-new', 'D3dxSkinManager Launcher.exe',
  (install) => {
    ...
    // Staged NEW topology (v4.0): D3dxSkinManager.exe is now the LAUNCHER (NOT in manifest); runtime → lib\.
    write(install, '.update/staged/D3dxSkinManager.exe', 'NEW-LAUNCHER');
    write(install, '.update/staged/lib/D3dxSkinManager.App.exe', 'APP-B');
    write(install, '.update/staged/data/en.json', 'EN-X2');
    write(install, '.update/staged/manifest.json', manifest('4.0', [
      ['lib/D3dxSkinManager.App.exe', 'APP-B'],
      ['data/en.json', 'EN-X2'],
    ]));
    write(install, '.update/ready.json', JSON.stringify({ version: '4.0' }));
  },
  (read, exists) => [
    // The crux: the new launcher lands at D3dxSkinManager.exe (copied — /XF only excluded the running
    // OLD launcher) AND is NOT deleted by the manifest-diff removal (guard on both launcher basenames).
    ['new launcher landed (D3dxSkinManager.exe = NEW-LAUNCHER)', exists('D3dxSkinManager.exe') && read('D3dxSkinManager.exe') === 'NEW-LAUNCHER'],
    ['runtime added (lib/App.exe = B)', exists('lib/D3dxSkinManager.App.exe') && read('lib/D3dxSkinManager.App.exe') === 'APP-B'],
    ['en.json updated (X2)', read('data/en.json') === 'EN-X2'],
    ['old launcher orphan left for the app cleanup step', exists('D3dxSkinManager Launcher.exe')],
