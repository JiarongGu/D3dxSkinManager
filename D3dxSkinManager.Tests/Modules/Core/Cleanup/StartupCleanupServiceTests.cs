...
/// <summary>
/// The CENTRALIZED startup cleanup/migration pipeline (Modules/Core/Cleanup): the runner executes
/// every registered IStartupCleanupStep in order, each isolated (one failure never blocks the rest).
/// Steps covered: stale managed downloads, orphaned update staging (kept when a complete pending
/// update exists), legacy process-state.json, legacy remote-sources/.cache.
/// </summary>
public class StartupCleanupServiceTests : IDisposable
{
    ...
    public StartupCleanupServiceTests()
    {
        ...
        globalPaths.Setup(p => p.RemoteSourcesDirectory).Returns(Path.Combine(_dataDir, "remote-sources"));
        ...
        _service = new StartupCleanupService(new IStartupCleanupStep[]
        {
            ...
            new LegacyRemoteIndexCacheCleanupStep(globalPaths.Object, logger),
        }, logger);
    }
    ...
    [Fact]
    public async Task RunAsync_RemovesLegacyRemoteIndexCacheDir()
    {
        var legacy = Path.Combine(_dataDir, "remote-sources", ".cache");
        Directory.CreateDirectory(legacy);
        File.WriteAllText(Path.Combine(legacy, "huihui_2.json"), "{}");

        await _service.RunAsync();

        Directory.Exists(legacy).Should().BeFalse("the v1 JSON index cache is superseded by the per-profile SQLite");
    }
