...
public class FileServiceTests : IDisposable
{
    ...
    [Fact]
    public async Task ExtractArchiveAsync_WithValidZip_ShouldExtractFiles()
    {
        // Arrange - Create a test ZIP archive
        var zipPath = Path.Combine(_testDataPath, "test.zip");
        var extractPath = Path.Combine(_testDataPath, "extracted");
        ...
        using (var archive = ZipFile.Open(zipPath, ZipArchiveMode.Create))
        {
            ...
            {
                await writer.WriteAsync("Content 1");
            }

            var entry2 = archive.CreateEntry("subfolder/file2.txt");
            using (var writer = new StreamWriter(entry2.Open()))
            {
                await writer.WriteAsync("Content 2");
            }
        }

        // Act
        var result = await _service.ExtractArchiveAsync(zipPath, extractPath);

        // Assert
        result.Should().BeTrue();
        File.Exists(Path.Combine(extractPath, "file1.txt")).Should().BeTrue();
        File.Exists(Path.Combine(extractPath, "subfolder", "file2.txt")).Should().BeTrue();
    ...
    [Fact]
    public async Task ExtractArchiveAsync_WithNonExistentArchive_ShouldThrowFileNotFoundException()
    {
        // Arrange
        var zipPath = Path.Combine(_testDataPath, "nonexistent.zip");
        var extractPath = Path.Combine(_testDataPath, "extracted");

        // Act & Assert
        await Assert.ThrowsAsync<FileNotFoundException>(() =>
            _service.ExtractArchiveAsync(zipPath, extractPath));
    }

    [Fact]
    public async Task ExtractArchiveAsync_WithNonExistentTargetDirectory_ShouldCreateDirectory()
    {
        // Arrange
        var zipPath = Path.Combine(_testDataPath, "test.zip");
        var extractPath = Path.Combine(_testDataPath, "new_folder", "extracted");
    ...
    [Fact]
    public async Task ExtractArchiveAsync_WithEmptyArchive_ShouldComplete()
    {
        // Arrange - Create empty ZIP
        var zipPath = Path.Combine(_testDataPath, "empty.zip");
        var extractPath = Path.Combine(_testDataPath, "extracted");
