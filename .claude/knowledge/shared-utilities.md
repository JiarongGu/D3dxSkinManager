...
| `canonicalJson` | `shared/utils/canonicalJson.ts` | Order-independent JSON string (recursively sorted object keys; arrays keep order) → dirty-tracking equality. Was duplicated as `canonical`/`canon` in the remote source + library editors (deduped 2026-07-13) |
...
- **2026-07-13** (remote cleanup pass): `canonical`/`canon` (byte-identical order-independent-JSON dirty-tracking) were duplicated in `RemoteSourceEditor` + `RemoteLibraryManagementScreen` → extracted to `shared/utils/canonicalJson.ts` (+ test). Same pass: 6× `JsonSerializerOptions` literals across the Remote services centralized to `RemoteJson` (Compact/Sparse/Pretty); the twin rule/alias filter→paginate blocks extracted to `PaginatedEditList` (+ test); the import↔index orchestration moved out of `RemoteFacade` into `RemoteIndexService.QueryAnnotatedAsync`/`MergeDetailTags`.
