...
- **Windows username** — `C:/Users/user\.nuget\...` baked into an old `.csproj` `<Reference Update=...>` HintPath (Visual Studio wrote the absolute path).
- **Dev-machine layout + sibling-project name** — `D3dxSkinManager\Games\<sibling>` (a sibling project's real name) + `D3dxSkinManager\...` in blobs **and commit-message bodies** across 12 commits.
...
- **No absolute local paths.** `D3dxSkinManager\Games\…`, `C:\Users\<name>\…`, mapped-drive roots. Use a repo-relative path, a neutral placeholder (`<repo>`, `%USERPROFILE%`), or move the real path to `local/`.
...
- **Edge cases where the rule does NOT apply:** fictional example paths (`C:\Games\MyGame`, `D:\Games\MyGame`, `E:/Games/ZZZ`) and generic UNC (`\\host\share`) in docs/tests are fine — they reveal nothing real. The maintainer's author email (`jiaronggu@hotmail.com`) is the public GitHub identity — intentional.
...
- **filter-repo literal replacement is CASE-SENSITIVE** — add every case as its own rule (`C:`/`c:`, `D:`/`d:`); a lowercase `D3dxSkinManager` survived a `D3dxSkinManager`-only pass.
