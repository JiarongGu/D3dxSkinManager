...
- **Beware resurrection anchors** (the trap that turns one purge into five). After you delete a tag/blob, OTHER refs quietly drag the old lineage back on the next filter-repo run or even a plain commit: `refs/stash` (rooted in an old pre-rewrite commit), `refs/remotes/origin/*` (old fetched state), `refs/original/*`, and reflogs. **Tell-tale:** `git rev-list --count --all` ≫ `git rev-list --count HEAD`. **Fix:** verify HEAD itself is clean first, then excise the anchors — `git stash clear && git update-ref -d refs/stash`, `git remote remove origin`, delete the tag, `git reflog expire --expire=now --all`, `git gc --prune=now` — until `--all` count **equals** the HEAD count. Re-add origin afterward.
